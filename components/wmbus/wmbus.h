...
namespace wmbus {
  ...
  class WMBusComponent : public Component {
    ...
    protected:
      ...
      void led_handler();
#ifdef USE_ESP_IDF
      bool tcp_send(const std::string &ip, uint16_t port, const uint8_t *data, size_t len);
      bool tcp_send_string(const std::string &ip, uint16_t port, const std::string &data);
      bool udp_send(const std::string &ip, uint16_t port, const uint8_t *data, size_t len);
      bool udp_send_string(const std::string &ip, uint16_t port, const std::string &data);
#endif
